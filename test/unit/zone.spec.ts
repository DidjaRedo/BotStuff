import { DefaultZoneOptions, Zone } from '../../src/zone';
import { Names } from '../../src/names';
import { PoiProperties, Poi, PoiLookupOptions } from '../../src/poi';

describe('Zone class', (): void => {
    ...
    describe('constructor', (): void => {
        it('should construct a default zone with just a name', () => {
            ...
            expect(zone.primaryKey).toEqual(Names.normalizeString(zoneName));
            expect(zone.keys.name).toEqual(Names.normalizeString(zoneName));
            expect(zone.options).toEqual(DefaultZoneOptions);
        });

        it('should construct a zone with options', () => {
            expect.assertions(5);
            const zoneName = 'Red Zone';
            const zone = new Zone(zoneName, { failForUnknownZones: true });
            expect(zone.name).toEqual(zoneName);
            expect(zone.primaryKey).toEqual(Names.normalizeString(zoneName));
            expect(zone.keys.name).toEqual(Names.normalizeString(zoneName));
        ...
        describe('with failForUnknownZones false', () => {
            it('should construct a zone with options and POIs, ignoring unknown zones', () => {
                expect.assertions(4);
                const zoneName = 'Red Zone';
                const zone = new Zone(zoneName, { failForUnknownZones: false }, pois);
                expect(zone.name).toEqual(zoneName);
                expect(zone.primaryKey).toEqual(Names.normalizeString(zoneName));
                expect(zone.keys.name).toEqual(Names.normalizeString(zoneName));
        ...
        describe('with failForUnknownZones true', () => {
            it('should construct a zone with options and pois if all POIs are from this zone', () => {
                expect.assertions(4);
                const zoneName = 'Red Zone';
                const zone = new Zone(zoneName, { failForUnknownZones: true }, redPois);
                expect(zone.name).toEqual(zoneName);
                expect(zone.primaryKey).toEqual(Names.normalizeString(zoneName));
                expect(zone.keys.name).toEqual(Names.normalizeString(zoneName));
                expect(zone.numPois).toEqual(redPois.length);
            ...
            it('should throw if any supplied POIs are from another zone', () => {
                ...
                expect(() => {
                    zone = new Zone(zoneName, { failForUnknownZones: true }, pois);
    ...
    describe('addPois method', () => {
        describe('with failForUnknownZones false', () => {
            it('should add POIS for this zone while ignoring unknown zones', () => {
                expect.assertions(2);
                const zoneName = 'Red Zone';
                const zone = new Zone(zoneName, { failForUnknownZones: false });
